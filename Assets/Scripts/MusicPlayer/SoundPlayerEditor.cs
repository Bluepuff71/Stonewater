...
public class SoundPlayerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (soundPlayer.tracks == null)
        {
            soundPlayer.tracks = new List<AudioClipWithVolume>();
        }
        ...
        if(soundPlayer.audioSource != null)
        {
            ...
            musicHeaderFoldout = EditorGUILayout.Foldout(musicHeaderFoldout, "Playlist");
            if (musicHeaderFoldout)
            {
                EditorGUI.indentLevel++;
                for (int i = 0; i < soundPlayer.tracks.Count; i++)
                {
